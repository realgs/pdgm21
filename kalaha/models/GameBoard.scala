...
import kalaha.utils.Constants.{PLAYER_HOLES, STONES_AMOUNT}
...
class GameBoard {
      ...
      turn match
        case First =>
          if !isSimulation then
            println(first_player_turn_normal)
        case Second =>
          if !isSimulation then
            println(second_player_turn_normal)
  ...
  def isMoveValid(pitIndex: Int): Boolean =
    //if(pitIndex < 0 || pitIndex > PLAYER_HOLES) return false
    turn match
      case First =>
