...
object Main {
  ...
  /*
    ...
    Poprawione zaczyna się w linijce 103,
    dodatkowo zmieniłem argument decimalToAnySystem na 2
  ...
  */
  ...
  def decimalToAnySystem(number: Int, system: Int): List[Int] =
  @tailrec
  ...
  if system <= 1 then throw new Exception("Wrong conversion system")
  else
  if number == 0 then List(0)
  else if number > 0 then decToAnySystemHelper(number, system, Nil)
  else -1::decToAnySystemHelper(-number, system, Nil)
  ...
  */
  ...
  // Poprawione dla przykładu który podałem

  sealed trait BTreeFriends[+A]
  case object EmptyFr extends BTreeFriends[Nothing]
  case class NodeFr[+A](value: A, listOfFriends: List[BTreeFriends[A]]) extends BTreeFriends[A]

  /*
    Stworzone drzewo nie-binarne, usuwane są z niego duplikaty,
    przetwarzane jest na drzewo binarne,
    będzie lepiej póżniej je przetwarzać (nie-binarne miałoby mały sens),
    a kolejność w tym wypadku nie bedzie ważna,tylko brak powtórzeń. Jednak
    w prawdziwej aplikacji, można by było posortować i stworzyć drzewo binarne,
    ze względu na np wiek.

  */

  //Dla drzew przyjaciół
  def deleteDuplicateDepth[A](tree: BTreeFriends[A]): BTree[A] =
  def generateList[A](stack: List[BTreeFriends[A]], elements: List[A]): List[A]=
    stack match
  case Nil => Nil
  case EmptyFr::tail => generateList(tail, elements)
  case NodeFr(value, listOfFriends)::tail =>
  if !listContains(elements, value) then value::generateList(listOfFriends:::tail, value::elements)
  else generateList(tail,elements)
  ...
  //
  def deleteDuplicatesBreadth[A](tree: BTreeFriends[A]): BTree[A] =
  def deleteHelper[A](queue: List[BTreeFriends[A]], visited: List[A]): List[A]=
    queue match
  case Nil => Nil
  case EmptyFr::tail => deleteHelper(tail, visited)
  case NodeFr(value, listOfFriends)::tail =>
  if listContains(visited, value) then deleteHelper(tail:::listOfFriends, visited)
  else value::deleteHelper(tail:::listOfFriends, value::visited)
  if tree != Empty then
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    ...
    println( decimalToAnySystem(31, 16) == List(1,15))
    println( decimalToAnySystem(-31, 16) == List(-1, 1, 15) )
    println( decimalToAnySystem(0, 16) == List(0) )
    println( decimalToAnySystem(100, 16) == List(6, 4)  )

    println()
    println("testy na system 2")
    println( decimalToAnySystem(31, 2)== List(1, 1, 1, 1, 1) )
    println( decimalToAnySystem(-31, 2) == List(-1, 1, 1, 1, 1, 1) )
    println( decimalToAnySystem(0, 2) == List(0) )
    println( decimalToAnySystem(100, 2) == List(1, 1, 0, 0, 1, 0, 0) )

    println()
    println("testy na system 5")
    println( decimalToAnySystem(3196, 5) == List(1, 0, 0, 2, 4, 1))
    println( decimalToAnySystem(-3196, 5) == List(-1, 1, 0, 0, 2, 4, 1) )
    println( decimalToAnySystem(0, 5) == List(0) )
    println( decimalToAnySystem(99, 5) == List(3, 4, 4) )
    ...
    val treeFriends =
      NodeFr(1,
        List( NodeFr(2, List()), NodeFr(3, List( NodeFr(2, List()), NodeFr(4, List( NodeFr(5, List( NodeFr(6, List()))), NodeFr(2, List()))), NodeFr(5, List( NodeFr(6, List()))) ))))

    println("test drzewa znajomych")
    println(deleteDuplicateDepth(treeFriends))
    println(deleteDuplicatesBreadth(treeFriends))
